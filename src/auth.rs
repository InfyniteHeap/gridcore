...
pub async fn request_microsoft_authorization_token(
    ...
    match &json::parse_from_string(&response).unwrap()["access_token"] {
...
pub async fn request_xbox_authentication_response(
    ...
    match &json::parse_from_string(&response).unwrap()["Token"] {
        Value::String(token) => Ok(token.to_owned()),
        _ => unreachable!(),
    }
}
...
pub async fn request_xsts_authorization_response(
    ...
    match (
        &json::parse_from_string(&response).unwrap()["Token"],
        &json::parse_from_string(&response).unwrap()["DisplayClaims"]["xui"][0]["uhs"],
    ) {
...
impl MinecraftProfile {
    ...
    pub async fn check_if_player_owns_game(&self) -> Result<bool, reqwest::Error> {
        ...
        if let Value::Array(items) = &json::parse_from_string(&response).unwrap()["items"] {
            if !items.is_empty() {
                Ok(true)
            } else {
                Ok(false)
    ...
    pub async fn request_uuid_and_username_response(&mut self) -> Result<(), reqwest::Error> {
        ...
        match (
            &json::parse_from_string(&response).unwrap()["name"],
            &json::parse_from_string(&response).unwrap()["id"],
        ) {
    ...
    pub fn save_to_file(&self) -> anyhow::Result<()> {
        let contents = json::convert_to_string(self)?;
